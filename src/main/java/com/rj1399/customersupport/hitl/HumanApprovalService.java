...
public class HumanApprovalService {
    ...
    public HumanApprovalService(JdbcTemplate jdbc,
    ...
    public Approval create(String orderNumber, BigDecimal amount, String reason, String executionId) {
        ...
        jdbc.update("INSERT INTO human_approvals (id, order_number, amount, reason, status, execution_id, created_at) VALUES (?, ?, ?, ?, 'PENDING', ?, ?)",
    ...
    public Approval approve(UUID id, String decidedBy, String decisionReason) {
        ...
        customerSupportService.createRefund(
                new ApiDtos.RefundRequest(
                        current.orderNumber(),
                        current.reason(),
                        "hitl-" + current.id()));

        jdbc.update("UPDATE human_approvals SET status='APPROVED', decided_at=?, decision_by=?, decision_reason=? WHERE id=? AND status='PENDING'",
    ...
    public Approval reject(UUID id, String decidedBy, String decisionReason) {
        Approval current = get(id);
        if (!"PENDING".equals(current.status())) return current;
        jdbc.update("UPDATE human_approvals SET status='REJECTED', decided_at=?, decision_by=?, decision_reason=? WHERE id=? AND status='PENDING'",
                Instant.now(), decidedBy, decisionReason, id);
    ...
    public record Approval(UUID id, String orderNumber, BigDecimal amount, String reason, String status,
                           String executionId, Instant createdAt, Instant decidedAt, String decisionBy,
