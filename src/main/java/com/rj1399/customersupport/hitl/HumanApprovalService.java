...
public class HumanApprovalService {
    ...
    public Approval create(
        ...
        try {
            ...
            Approval duplicate = findByIdempotencyKey(idempotencyKey);
            if (duplicate != null) {
                return duplicate;
            }
            throw ex;
        ...
        if (executionId != null) {
            traceStore.add(new AgentTrace(
                    executionId,
                    Instant.now(),
                    AgentTrace.TraceEventType.HUMAN_APPROVAL_REQUESTED,
                    "hitl",
                    "refund-approval",
                    0,
                    Map.of(
    ...
    public List<Approval> pending() {
        return jdbc.query(
                """
                SELECT id, order_number, amount, reason, idempotency_key, status,
                       execution_id, created_at, decided_at, decision_by, decision_reason
    ...
    public Approval approve(UUID id, String decidedBy, String decisionReason) {
        ...
        customerSupportService.createApprovedRefund(
        ...
        int updated = jdbc.update(
                ...
                SET status = ?, decided_at = ?, decision_by = ?, decision_reason = ?
                WHERE id = ? AND status = ?
                """,
                "APPROVED",
    ...
    private void publishDecision(
        ...
        if (approval.executionId() != null) {
            traceStore.add(new AgentTrace(
                    ...
                    Instant.now(),
                    AgentTrace.TraceEventType.HUMAN_APPROVAL_DECISION,
                    "hitl",
                    "refund-approval",
                    0,
                    Map.of(
